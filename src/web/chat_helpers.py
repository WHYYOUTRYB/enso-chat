...
- 每次预测工具返回的 `[数据来源]` 前言里，若 ENSO 数据偏旧（距今 >2 个月）会带 `⚠️ 数据偏旧` 标注。
- **看到该标注时**：告知用户「当前数据偏旧（截止 X 月），预测起算点滞后；后台已在刷新，下一轮对话会载入最新数据；如需立即用最新数据，可显式让我 `load_enso_data(refresh_noaa=True)`」。
- 不要因数据偏旧就拒绝预测——先用现有数据出预测并明示不确定性，同时提示刷新。
- 后台刷新是自动的（用户无需操作）；显式 `refresh_noaa=True` 会同步等待（约 5s，含重训）。
"""
